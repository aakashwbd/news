<?php
	...
	try {
		...
		$viewSave = $view->videoViwesave($id);

		$singleVideoShow = $video->singleVideoShow($id);
		// $row = $singleVideoShow->fetch(PDO::FETCH_ASSOC);
		// var_dump($row);

		$count = $singleVideoShow->rowCount();
        ...
        if ($count !== 0) {
            ...
            $response_data['view_video_count'] = (int) $view->videoViewCount($id);
        }	
