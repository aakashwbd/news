...
<script>
    ...
    $('#form').submit(function (e){
        e.preventDefault();
        let form = $(this);
        let apiURL = window.origin + '/api/v1/category/update.php?id=<?= $_GET['id']?>'
        formSubmit("patch", "submit-button", form, apiURL);
    })
    
    let getId = <?= $_GET['id']?>;
    
    let url = window.origin + "/api/v1/category/getEdit.php?id="+getId;
